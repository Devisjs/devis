...
class Core extends EventEmitter {
	...
	client(opt, callback) {
		if (typeof opt.id === "undefined") { callback("empty_id", null); return (log.error("empty_id", 1)); }
		if (this.clients[opt.id]) { callback("unique_id", null); return (log.error("unique_id", 1)); }
		else {
			this.clients[opt.id] = new Transport();
			this.clients[opt.id].client(opt, (err, res) => {
				if (callback) callback(err, res);
			});
			return this;
		}
	...
	getFunctions(arg0, arg1, arg2, arg3, callback) {//arg0: local or distant microservice; arg1: name, full path, or network opt for the microservice; arg2: folder name if arg1 is just a name
		//arg3 depend on your folder position, 1 for an ascending search and -1 for a descending search
		if (arg0 === "local") {
			if (arg1.split(__dirname) >= 2) {//a full path
				let devis = require(arg1);
