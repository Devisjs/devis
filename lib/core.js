...
class Core extends EventEmitter {
        ...
        if (res) {
            ...
            try {
                if (this.transport[id].state == 'connected') {
                    ...
                    this.transport[id].getCallback(this.ident, this.name, (err, result, id) => {
                    ...
                    let i = 0;
                    this.stateVerify(id, i,pattern, args,callback);
                }
            } catch (e) {
                new Error(e);
                callback(null, 0);
    ...
    stateVerify(id, i,pattern, arg,callbacks) {
        let res;
        process.nextTick(() => {
            i++;
            if (this.transport[id].state == 'connected') {
                ...
                if (i < 5) this.stateVerify(id, i,pattern, args,callback);
            }
        })
    }
